...
foreach ($service in $services) {
    ...
    if ($status -like "*$service*") {
        ...
        Write-Host "ERROR: $service is not running. Starting services..." -ForegroundColor Red
        docker-compose up -d
...
function Run-Pipeline {
    ...
    Write-Host "Running $PipelineName Pipeline..." -ForegroundColor Cyan
...
# Run data pipelines
Write-Host "Starting Dataset Population Process..." -ForegroundColor Green
...
if (Test-Path "data_pipeline/scripts/run_clinvar_pipeline.py") {
    ...
    Write-Host "ClinVar pipeline script not found" -ForegroundColor Yellow
}
...
if (Test-Path "data_pipeline/scripts/run_tcga_pipeline.py") {
    ...
    Write-Host "TCGA pipeline script not found" -ForegroundColor Yellow
}
...
if (Test-Path "data_pipeline/scripts/run_pubmed_pipeline.py") {
    ...
    Write-Host "PubMed pipeline script not found" -ForegroundColor Yellow
}

# Verify data population
Write-Host "Verifying Database Population..." -ForegroundColor Yellow
...
# Display summary
Write-Host "Dataset Population Summary:" -ForegroundColor Green
...
Write-Host "Database population completed successfully!" -ForegroundColor Green 